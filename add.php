...
if ( !$connection ) {
    ...
    if ( 'addSupervisor' == $action ) {
        ...
        $subsubstate = $_REQUEST['subsubstate'] ?? '';
        $password = $_REQUEST['password'] ?? '';

        if ($supervisor_id && $fname && $lname && $lname && $phone && $password && $address ) {
            $hashPassword = password_hash( $password, PASSWORD_BCRYPT );
            $query = "INSERT INTO supervisors(supervisor_id,fname,lname,email,phone,address,subsubstate,password,role) VALUES ('$supervisor_id','{$fname}','$lname','$email','$phone','$address','$subsubstate','$hashPassword','supervisor')";
        ...
        include 'upload.php';
            if(uploadFile($_FILES))
            {
                ...
                $query = "INSERT INTO house(house_id,house_doc,subsubstate,kebele_id) VALUES 
                        ('$house_id','$house_doc','$subsubstate','$kebele_id')";
                        //echo $query;
            mysqli_query( $connection, $query );
            echo "<script type=\"text/javascript\">
            alert(\"Updated Succesfully\"); 
