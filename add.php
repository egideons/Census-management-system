...
if ( !$connection ) {
        ...
        if ( isset( $_REQUEST[ 'resset' ] ) )
 {
        ...
        if ( $enumerator_id && $fname && $lname && $lname && $phone && $password && $address ) {
            $hashPassword = password_hash( $password, PASSWORD_BCRYPT );
            $query = "INSERT INTO enumerators(enumerator_id,fname,lname,email,phone,address,unit,password,role,supervisor_id) VALUES ('$enumerator_id','{$fname}','$lname','$email','$phone','$address','$unit','$hashPassword','enumerator','$sessionId')";
            mysqli_query( $connection, $query );
            //echo $query;
        ...
        if ( uploadFile( $_FILES ) )
 {
            ...
            $query = "INSERT INTO house(house_id,house_doc,subcity,unit_id) VALUES 
                        ('$house_id','$house_doc','$subcity','$unit_id')";
            //echo $query;
            mysqli_query( $connection, $query );
            echo '<script type=\'text/javascript\">
            alert(\"Updated Succesfully\"); 
