function validateform() {
  ...
  try {
    ...
    var phone = "07039502751";
  }
  try {
    var password =
      document.getElementsByName("password")[0].value ?? "A123456789a";
  } catch (error) {
    var password = "A123456789a";
  }
  try {
    var newpassword =
      document.getElementsByName("newPassword")[0].value ?? "A123456789a";
  } catch (error) {
    var newpassword = "A123456789a";
  }
  try {
    var confirmpassword =
      document.getElementsByName("confirmPassword")[0].value ?? "A123456789a";
  } catch (error) {
  ...
  if (
    ...
    alert(
      "Password must be minimum eight characters, at least one letter or at least one number"
    );
    return false;
